...
import (
	...
	"GoParallelDownload/core"
	"GoParallelDownload/downloader"
	"GoParallelDownload/pool"
	"GoParallelDownload/rate"
	"GoParallelDownload/state"
	"GoParallelDownload/types"
)

var Queues = make(map[int]*types.DownloadQueue)

func ProcessQueue(queue *types.DownloadQueue, pool *pool.WorkerPool, results chan<- types.DownloadStatus) {
...
func setupQueuePool(queue *types.DownloadQueue) (*pool.WorkerPool, error) {
	if len(queue.Tasks) == 0 {
		return nil, fmt.Errorf("queue has no tasks")
	}
	queue.TokenBucket = rate.NewTokenBucket(queue.SpeedLimit, queue.SpeedLimit)
	...
	return pool.NewWorkerPool(queue.ConcurrentLimit), nil
}
...
func submitQueueTasks(queue *types.DownloadQueue, queuePool *pool.WorkerPool, results chan<- types.DownloadStatus) {
	for _, task := range queue.Tasks {
		...
		queuePool.Submit(func() {
			downloader.DownloadFile(task, controlChan, results, queue)
			core.CleanupTask(task)
		})
	}
}
