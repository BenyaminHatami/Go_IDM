...
func main() {
	...
	pool := pool.NewWorkerPool(2)
	var wg sync.WaitGroup

	core.StartProgressDisplay(&wg)
	...
	core.MonitorDownloads(&wg, results, totalTasks)
...
func setupDownloadQueues() []*types.DownloadQueue {
	...
	for i := range queuesList {
		core.SetFileNames(queuesList[i].Tasks)
		core.SetFileType(queuesList[i].Tasks)
	}
	return queuesList
}
